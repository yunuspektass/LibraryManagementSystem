...
export default function BookDetailPage() {
  ...
  const { user } = useAuth();
  const { refreshBorrowedBooks } = useNotification();
  ...
  const handleBorrow = async () => {
    if (!book || !book.availableCopies || book.availableCopies <= 0) {
    ...
    try {
      ...
      // Profil sayfasındaki ödünç listesini yenile
  ...
  const handleReturn = async () => {
    try {
      ...
      const activeRecord = records.find(
        r => r.userId === user.id && r.bookId === book.id && !r.returnDate && !r.returnRequested
      );

      if (activeRecord) {
        await borrowRecordsAPI.requestReturn(activeRecord.id);
        alert("İade talebi oluşturuldu! Kütüphane personeli onayladığında kitap iade edilmiş olacak.");
        // Profil sayfasındaki listeyi yenile
        await refreshBorrowedBooks();
      } else {
        alert("Aktif ödünç kaydı bulunamadı veya zaten iade talebi oluşturulmuş.");
      }
    } catch (err) {
              ...
              {user?.role === 'LibraryStaff' ? (
                <div style={{
                  ...
                  <p style={{ margin: 0, fontSize: '1rem', fontWeight: '500' }}>
                    📚 Kütüphane personeli kitap ödünç alamaz
                  </p>
                </div>
              ) : !isBorrowed ? (
                <button
                  ...
                  disabled={!book.availableCopies || book.availableCopies <= 0 || loading}
                  style={{
